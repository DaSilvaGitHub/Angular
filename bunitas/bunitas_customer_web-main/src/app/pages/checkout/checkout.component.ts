...
export class CheckoutComponent implements OnInit {
  ...
  @ViewChild('idLocation') locationCollapse: CollapseComponent;
  @ViewChild('idDate') dateCollapse: CollapseComponent;
  @ViewChild('idPayment') paymentCollapse: CollapseComponent;
  ...
  calcPrices() {
    this.subtotal =
      this.serviceCart.totalPrice +
      this.serviceCart.deliveryCharge -
      this.serviceCart.discount;
  ...
  getSpecialist() {
    ...
    this.api
      .post_private('v1/specialist/getSpecialist', { id: uid })
      .then(
        (data: any) => {
  ...
  async calculateDistance(addressLat: any, addressLng: any, address) {
    ...
    if (distance > parseFloat(this.util.allowDistance)) {
      ...
      this.serviceCart.deliveryCharge = parseFloat(
        this.serviceCart.deliveryCharge
      ).toFixed(2);
  ...
  getSlotsForBookings() {
    ...
    this.slotList = [];
    this.api
      .post_private('v1/timeslots/getSlotsByForBookings', param)
      .then(
        (data: any) => {
          if (
            data &&
  ...
  selectSlot(item: any, startDate: any) {
    let combineString = `${this.selectedDate} ${startDate}`;
    let compareDate = new Date(combineString);
    const currentDate = new Date();
    if (compareDate <= currentDate) {
      this.util.errorMessage(this.util.translate('Please select Correct time'));
      return;
    }
  ...
  getFreelancerByID() {
    ...
    this.api
      .post('v1/salon/salonDetails', param)
      .then(
        (data: any) => {
          if (data && data.status == 200) {
            ...
            this.isServiceAtHome =
              data.data.service_at_home == 0 ? false : true;
            if (!this.isServiceAtHome) {
              this.serviceAt = 'salon';
            }
            this.freelancerAddress = data.data.address;
            console.info(this.freelancerAddress);
          }
        },
        (error) => {
          this.util.apiErrorHandler(error);
    ...
    if (ids.includes(this.storeInfo.uid)) {
      ...
      this.util.errorMessage(
        this.util.translate('This coupon is not valid for ') +
          this.storeInfo.user_info.first_name
      );
    }
  }
  ...
  async payWithInstaMOJOWeb() {
    ...
    const param = {
      ...
      address:
        this.serviceAt == 'home'
          ? JSON.stringify(this.serviceCart.deliveredAddress)
          : 'NA',
      items: localStorage.getItem('userCart'),
      coupon_id:
        this.serviceCart &&
        this.serviceCart.coupon &&
        this.serviceCart.coupon.id
          ? this.serviceCart.coupon.id
          : 0,
      coupon:
        this.serviceCart &&
        this.serviceCart.coupon &&
        this.serviceCart.coupon.id
          ? JSON.stringify(this.serviceCart.coupon)
          : 'NA',
      ...
      wallet_price:
        this.walletCheck == true && this.walletDiscount > 0
          ...
          : 0,
      notes:
        this.orderNotes == '' || this.orderNotes == null
          ? 'NA'
          : this.orderNotes,
      status: 8,
    };
    ...
    this.util.start();
    this.api
      .post_private('v1/appoinments/create', param)
      .then(
        (data: any) => {
          this.util.stop();
          if (
            ...
            const param = {
              ...
              redirect_url:
                this.api.baseUrl +
                'v1/instaMOJOWebSuccessAppointments?id=' +
                orderId,
              phone:
        ...
        (data: any) => {
          ...
          if (
            ...
            if (
              ...
              this.selectedCard = data.success.id;
              this.createPayment();
            } else {
      ...
      .then(
        (data: any) => {
          ...
          if (data && data.status && data.status == 200 && data.data) {
            ...
            var options = {
              amount: this.serviceCart.grandTotal
                ? this.serviceCart.grandTotal * 100
                : 5,
              email: this.getEmail(),
              logo:
