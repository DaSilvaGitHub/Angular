...
export class UtilService {
  ...
  public favoritesTreatments: any[];
  ...
  getSubTotalPrice(cost: number, distance: number, discount: number) {
    return cost + distance - (cost * discount) / 100;
  }

  getDepositFee(cost: number) {
    return ((cost * this.general.deposit_now) / 100).toFixed(2);
  }

  getProcessingFee(cost: number) {
    return (
      (((cost * this.general.deposit_now) / 100) *
        this.general.processing_fee) /
      100
    ).toFixed(2);
  }

  getTotalFee(cost: number) {
    return (
      this.general.booking_fee +
      this.getProcessingFee(cost) +
      this.getDepositFee(cost)
    );
  }
}
